...
const basicFantasyCartridge: Cartridge = {
  ...
  rules: {
    ...
    combat_event: function (sheet: State, context: import('../types').TurnContext): RuleOutcome {
      ...
      if (eventType === 'enemy_attack' || eventType === 'player_attack') {
        ...
        // Toggle in_combat if we are starting combat
        if (!sheet.stats['in_combat']) {
          sideEffects.push({
            what: 'entered combat mode',
            temp: false,
            impacts: [{ stats: 'in_combat', op: 'set', val: 1 }]
          });
        }
    ...
    attack: function (sheet: State, context: import('../types').TurnContext): RuleOutcome {
      ...
      if (!sheet.stats['in_combat']) {
        return {
          ruleDebugLogs: [`Action '${parentIntent.key}' requires being in combat mode.`],
    ...
    persuade: function (sheet: State, context: import('../types').TurnContext): RuleOutcome {
      ...
      if (sheet.stats['in_combat']) {
        return {
          ruleDebugLogs: [`Action '${parentIntent.key}' cannot be easily performed in combat mode.`],
          mustHappen: [`The player attempts to ${parentIntent.key}${parentIntent.what ? ' ' + parentIntent.what : ''}.`],
          mustNotHappen: [],
          mayHappen: [],
          stateMutations: []
