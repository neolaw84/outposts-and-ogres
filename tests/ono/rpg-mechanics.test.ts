...
describe('RPG Mechanics - Aspect Functions', () => {
  describe('drink_potion', () => {
    test('Healing potion restores HP', () => {
      ...
      const result = basicFantasyCartridge.rules['drink_potion'](sheet, { playerSignals: [], ruleKey: 'drink_potion', narrationSummary: {}, worldSignal: effect, typeCheck: typeCheck });
    ...
    test('Healing potion at full health returns no side effect', () => {
      const sheet = makeSheet({ stats: { ...basicFantasyCartridge.defaultState.stats, in_combat: 1 } }); // defense=5, hp=100, max_hp = 100
      const effect = {
        key: 'drink_potion',
        what: 'healing',
        meters: { potency: 5 }
      ...
      expect(result.mustHappen.join(' ')).toContain('already at full health');
    });
    ...
    test('Null effect returns default prompt', () => {
      const sheet = makeSheet();
      const result = basicFantasyCartridge.rules['drink_potion'](sheet, { playerSignals: [], ruleKey: 'drink_potion', narrationSummary: {}, worldSignal: null, typeCheck: null });

      expect(result.mustNotHappen.join(' ')).toContain('potion');
  ...
  describe('combat_event', () => {
    test('Enemy attack deals damage reduced by defense', () => {
      const sheet = makeSheet({ stats: { ...basicFantasyCartridge.defaultState.stats, in_combat: 1 } });
      const effect = {
        key: 'combat_event',
        what: 'enemy_attack',
        meters: { damage: 20 },
      ...
      const result = basicFantasyCartridge.rules['combat_event'](sheet, { playerSignals: [], ruleKey: 'combat_event', narrationSummary: {}, worldSignal: effect, typeCheck: typeCheck });
    ...
    test('Heavy damage causes stunned state', () => {
      ...
      expect(result.mustHappen.join(' ')).toContain('STUNNED');
    });
    ...
    test('Combat end awards gold and xp', () => {
      ...
      expect(result.mustHappen.join(' ')).toContain('25 gold');
      expect(result.mustHappen.join(' ')).toContain('100 XP');
    });

    test('Null effect returns ambient narration guide', () => {
      const sheet = makeSheet();
      const result = basicFantasyCartridge.rules['combat_event'](sheet, { playerSignals: [], ruleKey: 'combat_event', narrationSummary: {}, worldSignal: null, typeCheck: null });

      expect(result.mustNotHappen.join(' ')).toContain('combat');
      expect(result.stateMutations.length).toBe(0);
    });
  });
  ...
  describe('travel', () => {
    test('Travel ignores past timestamps', () => {
      ...
      const result = basicFantasyCartridge.rules['travel'](sheet, { playerSignals: [], ruleKey: 'travel', narrationSummary: {}, worldSignal: effect, typeCheck: typeCheck });
      // Should use current time since provided time is in the past
      expect(result.mustHappen.join(' ')).toContain('1000-01-01T08:00:00');
    });
    ...
    test('Null effect returns mustNotHappen for travel', () => {
      const sheet = makeSheet();
      const result = basicFantasyCartridge.rules['travel'](sheet, { playerSignals: [], ruleKey: 'travel', narrationSummary: {}, worldSignal: null, typeCheck: null });

      expect(result.mustHappen.length).toBe(0);
      expect(result.mustNotHappen.length).toBeGreaterThan(0);
      expect(result.stateMutations.length).toBe(0);
    });
  });
  ...
  describe('rest', () => {
    test('Short rest restores 25% HP', () => {
      ...
      const result = basicFantasyCartridge.rules['rest'](sheet, { playerSignals: [], ruleKey: 'rest', narrationSummary: {}, worldSignal: effect, typeCheck: typeCheck });

      const effects = result.stateMutations as Array<{ impacts: Array<{ stats: string; op: string; val: number }> }>;
      const hpImpact = effects[0].impacts.find(i => i.stats === 'hp');
      expect(hpImpact!.op).toBe('add');
      expect(hpImpact!.val).toBe(25); // 100 * 0.25 = 25
      expect(result.mustHappen.join(' ')).toContain('Awoke from rest');
    });
    ...
    test('Null effect returns no mustHappen entries', () => {
      const sheet = makeSheet();
      const result = basicFantasyCartridge.rules['rest'](sheet, { playerSignals: [], ruleKey: 'rest', narrationSummary: {}, worldSignal: null, typeCheck: null });
    ...
    describe('dynamically generated actions (e.g. attack)', () => {
      test('Attack triggers combat logic correctly on success', () => {
        const sheet = makeSheet({ stats: { ...basicFantasyCartridge.defaultState.stats, strength: 100, in_combat: 1 } }); // auto-success
        ...
        expect(result.ruleDebugLogs.some(l => l.includes('Rolled'))).toBe(true);
        expect(result.mustHappen.join(' ')).toContain('decisively');
      });

      test('Attack triggers combat logic correctly on failure', () => {
        const sheet = makeSheet({ stats: { ...basicFantasyCartridge.defaultState.stats, strength: 1, in_combat: 1 } }); // very weak
        const result = basicFantasyCartridge.rules['attack'](sheet, {
          playerSignals: [{ key: 'attack', what: 'goblin' }],
          ruleKey: 'attack',
          narrationSummary: {}, worldSignal: null, typeCheck: null
        ...
        expect(result.ruleDebugLogs.length).toBeGreaterThan(0);
      });
      ...
      test('Action used in wrong condition yields neutral outcome', () => {
        ...
        expect(result.ruleDebugLogs.some(l => l.includes('requires being in combat'))).toBe(true);
        expect(result.mustHappen[0]).toContain('attempts to attack');
      });
    });
  });
});
