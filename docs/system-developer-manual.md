...
4.  script.executeTurn(...)       →  run all rules, produce GamePlayEvent[] + effectInstructions
5.  adapter.saveState(newState)   →  persist updated state
6.  adapter.applyGamePlayOutput(events, state, effectInstructions)
...
| `effectInstructions` | `string` | Pre-formatted effect instruction text, generated by `generateEffectInstruction()` for each `WorldEventTracker`. Tells the LLM how to populate the `[NARRATION_SUMMARY]` JSON. |
...
3. Append the `effectInstructions` string. This tells the LLM how to populate the `"effects"` array in the `[NARRATION_SUMMARY]` JSON block when certain conditions are met.
...
> **Note:** The `effectInstructions` string is already fully formatted by the engine using `generateEffectInstruction()` from `src/utils/llm-utils.ts`. You don't need to parse or transform it — just inject it into the appropriate location in the platform's context.
